...
<script language="JavaScript" src="../flash/flash.js" type="text/javascript"></script><noscript><p>JavaScript</p></noscript>
<!-- 공통 : E -->
	...
	<div id="body">
		...
		<!-- 중간컨텐츠 : S -->
   		<div id="sub_contents"><a name="contents"></a>
			...
			<div id="con_head">
				<p class="position">
					<a href="../"><img src="../images/common/icon_home.gif" alt="HOME" /></a> 공무원센타 | 포탈리스트
				</p>
			</div>
			<!-- 현재위치 : E -->
			...
			<!-- 서브타이틀 : S -->
			<div id="sub_title">
				<h3 class="cont01"><!-- img src="../01_one/images/titile_01_0.gif" alt="공무원센타" / --></h3>
			</div>
			<!-- 서브타이틀 : E -->
	...
	<% if ( rEntity != null && rEntity.getRowCnt() != 0) { %>
		<% for (int i = 0; i < rEntity.getRowCnt(); i++ ) { %>
			<li>
				...
				<span class="subject2">
					 <%="<span class='t_name'>["+rEntity.getValue(i, "TITLE_HEADER")+"]</span>"+KJFUtil.java2html(KJFUtil.getTitleLimit(rEntity.getValue(i, "SUBJECT"), 25))%>
	...
	<% if ( rEtcEntity != null && rEtcEntity.getRowCnt() != 0) { %>
		<% for (int i = 0; i < rEntity.getRowCnt(); i++ ) { %>
			<li>
				...
				<span class="subject2">
					 <%="<span class='t_name'>["+rEtcEntity.getValue(i, "TITLE_HEADER")+"]</span>"+KJFUtil.java2html(KJFUtil.getTitleLimit(rEtcEntity.getValue(i, "SUBJECT"), 25))%>

...
public class BbsSetConf {
    ...
    public static void check(HttpServletRequest request) throws  Exception {
         ...
         * A : Admin      - 홈페이지 관리자
         * M : Manager    - 시도 관리자
         * - B : Control  - 시도 업무담당자(통신)
         * C : Control    - 시군구 업무 담당자(통신)
         * - D : Control  - 시도일반사용자 
         * - E : Control  - 시군구일반사용자
         * - F : Control  - 시군구 업무 담당자(기타)
         * - G : Control  - 시도 업무 담당자(기타)
         * - H : Control  - 행정안전부
         * - I : Control  - 방송통신위원회
         * U : User       - 개인회원
         * UE : enterprise - 기업회원
        ...
        // 게시판 정보 세팅 검색
        if ( ( status == null 
                || !KJFUtil.print(status.getCT_ID()).equals(CT_ID)) 
                    || ( !KJFUtil.isEmpty(user) && !status.getUSER_ID().equals(KJFUtil.print(user.getUSER_ID()) )
             ) )  {
            ...
            if (IsboardAdmin) {             
                ...
                status.setWRITE(getLevelChk(rEntity.getValue(0, "WRITE_LEVEL")));      // WRITE_LEVEL : 쓰기권한
                status.setREAD(getLevelChk(rEntity.getValue(0, "READ_LEVEL")));        // READ_LEVEL : 읽기권한
            }
    ...
    public static StatusEnt setBasicSatatus(StatusEnt status, ReportEntity rEntity, UserEnt user) throws Exception {
        ...
        status.setPUBLIC_DIV_YN(KJFUtil.yn2Boolean(rEntity.getValue(0, "PUBLIC_DIV_YN"))); // 공무원전용 게시판사용여부
    ...
    public static boolean getLevelChk(String level) throws Exception {        
        ...
        // 회원만 허용 : User
            ...
            if ( !"GU".equals(boardUserLevel) ) {
                return true;
            }            
        // 공무원만 허용 : PUBLIC
        } else if (level.equals("PU")) {
        	
            if ( "M".equals(boardUserLevel) ||
            	 "B".equals(boardUserLevel) ||
            	 "C".equals(boardUserLevel) ||
            	 "D".equals(boardUserLevel) ||
            	 "E".equals(boardUserLevel) ||
            	 "F".equals(boardUserLevel) ||
            	 "G".equals(boardUserLevel) ||
            	 "H".equals(boardUserLevel) ||
            	 "I".equals(boardUserLevel)
            	) {
                return true;
            }

...
public class BbsKListCmd implements KJFCommand {
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        String next = status.getURL();
        
        // 로그인 전용 게시판인지 체크(회원전용)
        if (status.isPUBLIC_DIV_YN()){
        	user = (UserEnt)request.getSession().getAttribute("user");
        	if (!KJFUtil.isEmpty(user)){
	    		if (
	        			"U".equals(user.getCAPITAL()) || 
	        			"UE".equals(user.getCAPITAL()) || 
	        			"".equals(user.getCAPITAL())
	        		){       
	        		throw new MsgException(request, "해당 메뉴에 대한 리스트조회 권한이 없습니다.","Y", "../index/IndexAction.do?cmd=Index");
	        	}
        	} else {
        		throw new LoginException(request, "회원전용 페이지 로그인 체크(권한)", "../member/login.do");
        	}
        } else{
	        if ( !status.isVIEW_LIST() ) {
    ...
    private void loadCondition(HttpServletRequest request, BbsParam pm) throws Exception {
        ...
        if ("faq_basic_pub".equals(status.getTYPE())) {
            ...
            sbSQL.append(" SELECT CODE,CODE_NAME 		 			\n");
            sbSQL.append(" FROM PT_BBS_COM_CODE          			\n");
            sbSQL.append(" WHERE CODE_GROUP ='004' AND BIGO ='000' AND USE_YN ='Y' \n");
            ...
            sbSQL.append(" SELECT CODE,CODE_NAME 		 			\n");
            sbSQL.append(" FROM PT_BBS_COM_CODE          			\n");
            sbSQL.append(" WHERE CODE_GROUP ='004' AND BIGO ='"+ KJFUtil.print(pm.getScFAQ_L_CATE(),"ALL")+"' AND USE_YN ='Y' \n");
            sbSQL.append(" ORDER BY ORDER_NUM ASC 	    			\n");           
...
//        if(status.isSD_DIV_YN()) {            
//            sbSelectSQL.append("       (SELECT CODE_NAME FROM PT_AREA_COM_CODE WHERE CODE = A.SD_CD) AS SD_NM,          \n");
//        }
        ...
        if (status.isONE_LINE()){
        	sbSelectSQL.append(" (SELECT COUNT(*) AS CNT FROM PT_BBS_COM_COMMENT WHERE CT_ID = A.CT_ID AND BOARD_SEQ = A.BOARD_SEQ) AS COMMENT_CNT,  \n");
        }

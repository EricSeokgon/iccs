...
/**
 * <p>Title       : BbsKWriteCmd Class</p>
 * <p>Description : 게시글 쓰기,수정 처리 클래스</p>
 * <p>Copyright   : Copyright(c) 2007 PKT. All rights reserved.</p>
 * 
 * NOTE : 게시판의 게시글 쓰기,수정 처리를 한다.
 * 
 * @version 1.0
 * @author  PKT
 * 
...
public class BbsKWriteCmd implements KJFCommand {
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        if (status.isPUBLIC_DIV_YN()){
	        if ( !status.isWRITE() ) {
	        	throw new MsgException(request, "해당 메뉴에 대한 작성 권한이 없습니다.","Y", "../index/IndexAction.do?cmd=Index");
	        }        	
...
//        }
    ...
    private void LoadData(HttpServletRequest request, BbsParam pm) throws Exception {
        ...
        sbSelectSQL.append("       (SELECT SIDO_NM FROM PT_SIDO_CODE WHERE AREA_CODE = PT_BBS_"+CT_ID+".SIDO_CODE) AS SIDO_NM,      \n");
        sbSelectSQL.append("       SIGUNGU_CODE,   \n");
        sbSelectSQL.append("       (SELECT SIGUNGU_NM FROM PT_SIDO_CODE WHERE AREA_CODE = PT_BBS_"+CT_ID+".SIGUNGU_CODE) AS SIGUNGU_NM,   \n");

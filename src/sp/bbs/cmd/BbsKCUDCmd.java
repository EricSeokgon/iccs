...
public class BbsKCUDCmd implements KJFCommand {
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        if(cmd.equals("BbsKC")) {            // 게시글 등록
            insertBbsKC(request);
    	} else if (cmd.equals("BbsKCC")) {   // 댓글 등록
            insertComment(request);
        } else if (cmd.equals("BbsKU")) {    // 게시글 수정
            updateBbsKU(request);
    	} else if (cmd.equals("BbsKCU")) {   // 댓글 수정
    		updateComment(request);
        } else if (cmd.equals("BbsKD")) {    // 게시글 삭제           
            deleteBbsKD(request);    
    	} else if (cmd.equals("BbsKDC")) {   // 댓글 삭제
        ...
        if (!cmd.equals("BbsKD") && status.getATT_NUM() > 0) {
            ...
            //setBBS_FILE(request, form);  <-- 기존꺼
           
        	String file_yn = KJFUtil.print(request.getParameter("file_yn"),"Y");
        	
        	if("Y".equals(file_yn)) {
        		BbsFileSet(request); //신규 dextupload 에 따라 수정
        	}
     ...
     * @throws Exception
     */
    private void BbsFileSet(HttpServletRequest request) throws Exception {
        
    	int fileCnt	= KJFUtil.str2int(request.getParameter("fileCnt"));
    	...
    	for(int i=0; i < fileCnt; i++) {
    		file_real_Name = request.getParameter("file_real_Name"+i);
    		file_sys_Name = request.getParameter("file_sys_Name"+i);
    		file_size = request.getParameter("file_size"+i);
    		
    		saveAttach(CT_ID, BOARD_SEQ, file_real_Name, file_sys_Name, file_size);
    	}
     ...
     * @throws Exception
     */
    private void saveAttach(String CT_ID, String BOARD_SEQ, String FILE_REAL_NAME, String SYS_FILE_NAME, String FILE_SIZE) throws Exception {
    ...
    private void insertBbsKC(HttpServletRequest request) throws Exception {
        ...
        rDAO.setValue(++i, request.getParameter("CONTENT").replaceAll("@@@@@", "&nbsp;"));
        rDAO.setValue(++i, "N");
    ...
    private void insertBbsKR(HttpServletRequest request) throws Exception {
        ...
        rDAO.setValue(++i, KJFUtil.print(request.getParameter("SIDO_CODE"),"").trim());
        rDAO.setValue(++i, KJFUtil.print(request.getParameter("SIGUNGU_CODE"),"").trim());
    ...
    */
    ...
    private void deleteComFiles(HttpServletRequest request) throws Exception {
        ...
        KJFFile.deleteFile(Config.props.get("BBS_FILE_IMG") + CT_ID + KJFFile.FILE_S, SYS_FILE_NAME); // dir에서 파일을 삭제한다.
    }
     ...
     * @throws Exception
     **********************************************************************/
    private void insertComment(HttpServletRequest request) throws Exception {

        // 권한 체크
    	String id =  KJFUtil.print(status.getUSER_ID(),"");
        ...
        StringBuffer sbSQL = new StringBuffer();        
        sbSQL.append(" INSERT INTO PT_BBS_COM_COMMENT (		\n");
        sbSQL.append("        COMMENT_SEQ,         			\n");
        sbSQL.append("        CT_ID,               			\n");
        sbSQL.append("        BOARD_SEQ,             		\n");
        sbSQL.append("        SIDO_CODE,           			\n");
        sbSQL.append("        SIGUNGU_CODE,             	\n");
        ...
        rDAO.setValue(i++, getMaxCommentSeq());
        rDAO.setValue(i++, CT_ID);
        rDAO.setValue(i++, request.getParameter("BOARD_SEQ"));
        rDAO.setValue(i++, KJFUtil.print(request.getParameter("SIDO_CODE"),""));
        rDAO.setValue(i++, KJFUtil.print(request.getParameter("SIGUNGU_CODE"),""));
        rDAO.setValue(i++, status.getUSER_NAME());
        rDAO.setValue(i++, status.getUSER_ID());
        rDAO.setValue(i++, request.getParameter("COMMET"));
        rDAO.setValue(i++, KJFDate.getCurTime("yyyy-MM-dd HH:mm:ss"));
        rDAO.setValue(i++, KJFDate.getCurTime("yyyy-MM-dd HH:mm:ss"));
    ...
    private void updateComment(HttpServletRequest request) throws Exception {
        ...
        StringBuffer sbSQL = new StringBuffer();        
        sbSQL.append(" UPDATE PT_BBS_COM_COMMENT		\n");
        sbSQL.append(" SET   COMMENT_MEMO  = ?    		\n");        
        sbSQL.append(" WHERE COMMENT_SEQ  = ?      		\n");
   
        int i = 1;
        
        rDAO.setValue(i++, request.getParameter("COMMET_SUB"));
        rDAO.setValue(i++, request.getParameter("COMMENT_SEQ"));
     ...
     * @throws Exception
     ***********************************************************************/
    private void deleteComment(HttpServletRequest request) throws Exception {
     ...
     * @throws Exception
     *********************************************************************/
    private String getMaxCommentSeq() throws Exception {

        String sql	=
        	" SELECT " + KJFDBUtil.makeSql_nullFunc() +
        	"( MAX( COMMENT_SEQ), 0 ) + 1 MAXSEQ        \n" +
            "   FROM PT_BBS_COM_COMMENT                 \n";

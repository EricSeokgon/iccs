...
/**
 * <p>Title       : BoardSearchCmd Class</p>
 * <p>Description : 게시물 검색 처리 한다.</p>
 * <p>Copyright   : Copyright(c) 2007 PKT. All rights reserved.</p>
 * 
 * NOTE : 게시물 검색 정보를 처리를 한다. 
 * 
 * @version 1.0
 * @author  PKT
 */
public class BoardSearchCmd implements KJFCommand {
    
    public BoardSearchCmd() {
    }
    ...
    private void loadList(HttpServletRequest request, SearchParam pm)throws Exception {
        ...
        if ( KJFUtil.isEmpty( KJFUtil.print(pm.getScTextValue()) ) ) {
            ...
            xmllist = scWorker.loadXmlList(request, pm, searchUrl.toString(), "board");    
        }       
       
        System.out.println("사이즈" + xmllist.size());

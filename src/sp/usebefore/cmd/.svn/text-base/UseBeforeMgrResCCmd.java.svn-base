 ...
 * @author  PKT
 **************************************************************************/
public class UseBeforeMgrResCCmd implements KJFCommand {
    
	UserEnt user;
	
    public UseBeforeMgrResCCmd() {
    ...
    public void loadUserBeforeExe(HttpServletRequest request, UseBeforeParam pm, String key ) throws Exception {
        ...
        if ("2".equals(key)){
        	sbSQL.append(" MIDCLAS||' '|| SMACLAS AS  CLASS_NAME_CHK, CASE WHEN INSTR(SMACLAS,'-') > 0 THEN RTRIM(SUBSTR(SMACLAS,0,INSTR(SMACLAS,'-')-1)) "); 
        	sbSQL.append(" ELSE SMACLAS END CLASS_NAME1,  ");
        	sbSQL.append(" CASE WHEN INSTR(SMACLAS,'-') > 0 THEN RTRIM(SUBSTR(SMACLAS,INSTR(SMACLAS,'-')+1)) "); 
        ...
        if ("2".equals(key)){
        	sbSQLCnt.append(" SELECT CASE WHEN INSTR(SMACLAS,'-') > 0 THEN RTRIM(SUBSTR(SMACLAS,0,INSTR(SMACLAS,'-')-1)) "); 
    ...
    /**
     * 사용전검사 현장검사관리 조회 사용안함
     * 
     * @param request
     * @param form
     * @throws Exception
     */
//    public void loadUserBeforeExeBef(HttpServletRequest request, UseBeforeParam pm, String key ) throws Exception {
//                
//        ReportDAO rDAO = new ReportDAO();      
//        ReportEntity rEntTable1 = null;
//        ReportEntity rEntTable2 = null;
